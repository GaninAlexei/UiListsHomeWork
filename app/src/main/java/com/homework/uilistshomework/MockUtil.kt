...
object MockUtil {
    ...
    private fun getLikedList(context: MainActivity) =  listOf(
    ...
    private fun getUnlikedList(context: MainActivity) = listOf(
    ...
    fun getHeaderMoviesList(context: MainActivity) = listOf(Item.Header("Новинки кино"))+
            getMoviesNewList(context, 5) +
            Item.Header("Старые фильмы")+
            getMoviesOldList(context, 18)


    @ExperimentalStdlibApi
    fun getMoviesNewList(context: MainActivity, size: Int = newMovies.size) = buildList(size) {
    ...
    @ExperimentalStdlibApi
    fun getMoviesOldList(context: MainActivity, size: Int = newMovies.size) = buildList(size) {
    ...
    @ExperimentalStdlibApi
    fun getHeaderWatchedMoviesList(context: MainActivity) = listOf(Item.Header("Понравившиеся фильмы"))+
            getLikedList(context) +
            Item.Header("Непонравившиеся фильмы")+
            getUnlikedList(context)
}
